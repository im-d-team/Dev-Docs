...
<img src="https://user-images.githubusercontent.com/24274424/117546192-322d9b80-b064-11eb-8ce4-022f02896479.png" alt="3-21">

처음에 부모 프로세스와 자식 프로세스 모두 Pipe의 사용되지 않은 끝을 닫는다. 

그림 3.21에 표시된 프로그램에는 이 작업이 필요하지 않지만 작성자가 파일의 끝을 닫았을 때 Pipe에서 읽는 프로세스가 파일 끝 (read () return 0)을 감지 할 수 있는지 확인하는 것은 Pipe의 중요한 단계다.

Windows 시스템의 Ordinary pipes를 Anonymous pipes라고하며 UNIX와 유사하게 작동한다. 
단방향이며 통신 프로세스간에 부모-자식 관계를 사용한다. 또한 Pipe 읽기 및 쓰기는 일반 `ReadFile ()` 및 `WriteFile ()` 함수를 사용하여 수행할 수 있다. 
...
Ordinary pipes에는 UNIX 및 Windows 시스템 모두에서 통신 프로세스간에 상위-하위 관계가 필요한다. 즉, Pipe는 동일한 시스템의 프로세스 간 통신에만 사용할 수 있다.

<img src="https://user-images.githubusercontent.com/24274424/117546188-3063d800-b064-11eb-8323-b70a389503ad.png" alt="3-24">

<img src="https://user-images.githubusercontent.com/24274424/117546182-2d68e780-b064-11eb-9547-ed0b24e70c01.png" alt="3-25">
...
UNIX 시스템에서 Named pipes는 FIFO라고 한다. 생성되면 파일 시스템에서 일반적인 파일로 나타난다. FIFO는 `mkfifo ()` 시스템 호출로 생성되고 일반적인 `open ()`, `read ()`, `write ()` 및 `close ()` 시스템 호출로 조작된다.
...
전이중 통신이 허용되며 통신 프로세스는 동일하거나 다른 시스템에 상주할 수 있다. UNIX FIFO는 byte-oriented 데이터만 전송할 수 있는 반면, Windows 시스템은 byte-oriented 또는 message-oriented 데이터를 허용한다. `CreateNamedPipe ()` 함수로 생성되며 클라이언트는 `ConnectNamedPipe ()`를 사용하여 Named pipes에 연결할 수 있다. Named pipes를 통한 통신은 `ReadFile ()` 및 `WriteFile ()` 함수를 사용하여 수행 할 수 있다.
