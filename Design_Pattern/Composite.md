...
- 클라이언트가 단일 객체와 집합 객체를 구분하지 않고 동일한 형태로 사용하고자 경우

- 단일 객체(Single Instance)와 복합 객체(Group of Instance)를 같은 방법으로 취급하는 방식으로 , 개별적인 객체들과 복합 객체의 처리 방법 차이가 없을 경우

## 패턴의 장단점

### 장점:

- 클라이언트는 복합 객체를 사용하고 있는지, 단일 객체를 사용하고 있는지에 대해서 신경 쓰지 않아도 되므로 단순화시킬 수 있다

- 복합 객체를 하나의 단일화된 객체처럼 취급 할 수 있으므로 "일괄적인 관리"시 편리하다
