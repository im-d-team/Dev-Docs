...
>Composite 패턴은 상속을 이용하여 하위 클래스를 생성하는 방법으로 확장을 하여 오버라이딩을 통해 기능을 전부 명시해 줘야 하는 반면, >Decorator 패턴은 기본 객체에 추가하는 객체를 생성하는 방법으로 확장을 하여 기본 기능은 놔두고 추가되는 기능만 명시가 가능하다. 
